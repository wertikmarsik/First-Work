...
function LitecoinTrade() {
    ...
    const getBTC = () => {
        axios
            .get(`https://min-api.cryptocompare.com/data/v2/histoday?fsym=LTC&tsym=USD&limit=${limit}`)
            .then((response) => {
    ...
    const getCurrency = async () => {
        try {
            const response = await axios.get('http://localhost:3000/currency/getETH')
                    ...
                    <div className="trade-crypto-wrapper">
                        <div className="graphic-wrapper">
                            <div className="trade-panel">
                                ...
                                <div
                                    className="crypto-name-block-litecoin"
                                    useref={buttonCrypto}
                            ...
                            <div>
                                <h2>Litecoin Price Chart</h2>
                                {chartData ? <Line data={chartData}/> : <p>Loading chart...</p>}
                            </div>
                        </div>
                        <div className="crypto-calculator">
                            {selectedValue === "value1" && (
                                <div className="price-block-litecoin">1 LTC = USD $ {currency.USD}</div>
                            )}
                            {selectedValue === "value2" && (
                                <div className="price-block-litecoin">1 LTC = UAH ₴ {currency.UAH}</div>
                            )}
                            {selectedValue === "value3" && (
                                <div className="price-block-litecoin">1 LTC = EUR € {currency.EUR}</div>
                            )}
                            <div className="calculator"></div>
                            <p>LTC Price Calculator</p>
                        </div>
                        <div className="crypto-description">
                            <h3>LTC Price Live Data</h3>
                            <div>
                                The live price of Litecoin is $ 91.39 per (LTC / USD) today with
                                a current market cap of $ 6.64B USD. 24-hour trading volume is $
                                424.50M USD. LTC to USD price is updated in real-time. Litecoin
                                is -0.7% in the last 24 hours. It has a circulating supply of
                                72.65M USD.
                            </div>
                        </div>
