...
function EthereumTrade() {
    ...
    const getBTC = () => {
        axios
            .get(`https://min-api.cryptocompare.com/data/v2/histoday?fsym=ETH&tsym=USD&limit=${limit}`)
            .then((response) => {
                    ...
                    <div className="trade-crypto-wrapper">
                        <div className="graphic-wrapper">
                            <div className="trade-panel">
                                ...
                                <div
                                    className="crypto-name-block-ethereum"
                                    useref={buttonCrypto}
                        ...
                        <div className="crypto-calculator">
                            {selectedValue === "value1" && (
                                <div className="price-block-ethereum">1 ETH = USD $ {currency.USD}</div>
                            )}
                            {selectedValue === "value2" && (
                                <div className="price-block-ethereum">1 ETH = UAH ₴ {currency.UAH}</div>
                            )}
                            {selectedValue === "value3" && (
                                <div className="price-block-ethereum">1 ETH = EUR € {currency.EUR}</div>
                            )}
                            <div className="calculator"></div>
                            <p>ETH Price Calculator</p>
                        </div>
                        <div className="crypto-description">
                            <h3>ETH Price Live Data</h3>
                            <div>
                                The live price of Ethereum is $ 1,877.36 per (ETH / USD) today
                                with a current market cap of $ 226.14B USD. 24-hour trading
                                volume is $ 8.58B USD. ETH to USD price is updated in real-time.
                                Ethereum is -0.85% in the last 24 hours. It has a circulating
                                supply of 120.46M USD.
                            </div>
                        </div>
